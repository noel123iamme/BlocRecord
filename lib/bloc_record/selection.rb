...
module Selection 
  ...
  def where(args=[], ids={}, operator="=")
    ...
    if not args.nil?
      ...
      when Hash 
        ...
        expression = expression_hash.map {|key, value| "#{key} #{operator} #{BlocRecord::Utility.sql_strings(value)}"}.join(" and ")
      end
    end
    sql = <<-SQL 
  ...
  def not(args=[], ids={})
    where(args=[], ids={}, "<>") unless args.empty?
  end
  ...
  #     case args
  #     when String
  #       arg_hash["expression"] = args
  #     when Array 
  #       arg_hash["expression"] = args.shift
  #       arg_hash["params"] = args
  #     when Hash 
  #       expression_hash = BlocRecord::Utility.convert_keys(args)
  #       arg_hash["expression"] = expression_hash.map {|key, value| "#{key} = #{BlocRecord::Utility.sql_strings(value)}"}.join(" and ")
  #     end
  #   end
  #   arg_hash
  # end
